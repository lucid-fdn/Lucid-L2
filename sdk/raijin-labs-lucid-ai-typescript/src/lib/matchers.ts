...
import { LucidDefaultError } from "../models/errors/luciddefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | LucidDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | LucidDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LucidDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LucidDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
