...
import { getTokenLauncher } from '../shares';
import { getPassportManager } from '../services/passport/passportManager';
import { getPermanentStorage } from '../storage/depin';
...
shareRouter.post('/v1/passports/:id/token/launch', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : 'Token launch failed' });
  }
});
...
shareRouter.get('/v1/passports/:id/token', async (req, res) => {
  try {
    ...
    if (!info) {
      return res.status(404).json({ error: 'No share token found for this passport' });
    }

    res.json(info);
  } catch (error) {
    res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to get token info' });
  }
});
...
shareRouter.post('/v1/passports/:id/token/airdrop', async (req, res) => {
  try {
    ...
    if (!tokenMint) {
      return res.status(400).json({ error: 'No share token found for this passport. Launch a token first.' });
    }

    const { runRevenueAirdrop } = require('../jobs/revenueAirdrop');
    const result = await runRevenueAirdrop(passportId, tokenMint, Number(amountLamports));

    res.json(result);
  } catch (error) {
    res.status(500).json({ error: error instanceof Error ? error.message : 'Airdrop failed' });
  }
});
