// offchain/packages/engine/src/deployment/webhooks/normalizers/phala.ts
// Phala Cloud webhook normalizer

import type { IProviderNormalizer, NormalizedProviderEvent } from '../types';

export class PhalaNormalizer implements IProviderNormalizer {
  normalize(body: unknown, _headers: Record<string, string>): NormalizedProviderEvent {
    const payload = body as Record<string, unknown>;

    return {
      provider: 'phala',
      provider_deployment_id: String(payload.app_id || payload.cvm_id || payload.deployment_id || ''),
